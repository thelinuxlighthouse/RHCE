...
## Added Review: Privilege Escalation Is Not Authentication

SSH authentication answers:

```text
Can Ansible log in as the remote user?
```

Privilege escalation answers:

```text
After login, can that user run root-level tasks safely?
```

These are separate. A working SSH login does not mean `become: true` works.
...
When writing sudoers content with Ansible, always validate it before installing it:

```yaml
- name: Install sudoers rule for ansible user safely
...
4. **`become: true` at play level**: Adding `become: true` to a play enables privilege escalation for all tasks in that play by default.

5. **Task-level `become` overrides play-level**: A task can explicitly disable `become` even if the play has `become: true`.
...
1. Create or verify `ansible.cfg` with `become: true` and `become_ask_pass: false`
...
|---|---|---|
| `become: true` | Enable privilege escalation | `become: true` |
...
3. How does the `become: true` parameter in a play affect individual tasks?
...
3. The `become: true` parameter in a play enables privilege escalation by default for all tasks in that play. However, individual tasks can override this by explicitly setting `become: no` to disable escalation for that specific task. A task with `become: no` in a play with `become: true` will run without privilege escalation.
...
# Chapter 06 — Reviewed Edition Closing Checkpoint
