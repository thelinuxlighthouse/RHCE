...
## Added Review: RHCSA-to-Ansible Translation Matrix

Use this table as your daily translation habit:

| RHCSA target state | Ansible pattern |
|---|---|
| Package installed | `ansible.builtin.dnf: name=<pkg> state=present` or verified `dnf5` module if required |
| Service running now | `ansible.builtin.systemd_service: name=<svc> state=started` |
| Service starts at boot | `ansible.builtin.systemd_service: name=<svc> enabled=true` |
| Firewall service open | verified firewalld module from available collection, or controlled command fallback if required |
| User exists | `ansible.builtin.user: name=<user> state=present` |
| Group exists | `ansible.builtin.group: name=<group> state=present` |
| Directory exists | `ansible.builtin.file: path=<dir> state=directory` |
| Static config deployed | `ansible.builtin.copy` with owner/group/mode/backup as needed |
| Dynamic config deployed | `ansible.builtin.template` with variables and `validate:` where available |
| Line exists in file | `ansible.builtin.lineinfile` with correct `regexp` |
| Block exists in file | `ansible.builtin.blockinfile` with markers |
| Filesystem mounted now and after reboot | module-backed filesystem/mount workflow verified in the environment |
| Cron job exists | `ansible.builtin.cron` |
| Sensitive variable protected | `ansible-vault` and encrypted variable files |

For any module that is not present in `ansible.builtin`, verify the collection before using it:

```bash
ansible-doc -l | grep -E 'firewalld|lvg|lvol|sefcontext'
...
|---|---|---|
| Install packages | `ansible.builtin.dnf` / verified `ansible.builtin.dnf5`, and only environment-verified module-stream methods | `name`, `state: present` |
...
ansible-doc dnf_module  # verify first; do not assume this module exists
...
ansible all -m firewalld -a "service=ssh permanent=true immediate=true state=enabled"
...
ansible all -m seboolean -a "name=httpd_can_network_connect state=true persistent=yes"
...
This playbook demonstrates package management using the `dnf` module. The `update_cache: yes` parameter forces a metadata refresh. The `state: present` parameter ensures packages are installed without upgrading. DNF module-stream handling is environment-sensitive and must be verified before use. The `copy` module creates custom repositories.
...
- **Software packages**: Use `ansible.builtin.dnf` or a verified package-management module available in the execution environment.
...
# Chapter 13 — Reviewed Edition Closing Checkpoint
