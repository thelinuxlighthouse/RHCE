...
## Added Review: The Core Ansible Model in One Page

Ansible has a small number of moving parts. Once they become natural, playbooks become much easier to read.

```text
Inventory says WHERE.
Play says WHO and HOW.
Task says WHAT.
Module does the work.
Variables make it reusable.
Facts make it aware of the managed node.
Loops repeat the same idea.
Conditionals decide whether the idea applies.
Handlers react only when notified by change.
Roles package the pattern for reuse.
Collections deliver modules, roles, plugins, and documentation.
```

### Correct YAML inventory example

A YAML inventory must use mappings and lists correctly. This is safe structure:
...
Read it as: `all` contains groups, `production` contains child groups, and each group may contain hosts and variables.

### Modern loop guidance

Use `loop:` for normal lists. Know that older examples may show `with_items:`, but write `loop:` unless the exact older lookup-style loop is specifically needed.
...
| Package | `ansible.builtin.dnf` / verified `ansible.builtin.dnf5`, and only environment-verified module-stream methods, `yum` | Software installation |
...
| Error ignore | `ignore_errors: true` | Continue on failure |
...
| Privilege | `become: true` | Escalate to root |
...
# Chapter 02 — Reviewed Edition Closing Checkpoint
