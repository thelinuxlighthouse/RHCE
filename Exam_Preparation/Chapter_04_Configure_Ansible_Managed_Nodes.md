...
|---|---|---|
| Packages | `dnf`, `dnf_module` | Install and manage software |
...
| Scheduling | `cron`, `systemd_timer` | Schedule recurring tasks |
...
- **Software packages** are managed with the `dnf` and `dnf_module` modules. Update the cache before installing, and check for dependencies.
...
2. How does the `dnf_module` module differ from the `dnf` module, and when would you use each?
...
2. The `dnf` module manages individual packages and can handle both installation and updates. The `dnf_module` module manages DNF module streams, which allow you to enable specific versions of software (e.g., `postgresql:16`). Use `dnf` for standard package installation and `dnf_module` when you need to enable specific software versions through DNF modules.
