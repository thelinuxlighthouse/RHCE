...
## Added Review: Templates and Vault Make Playbooks Real

Templates solve this problem:

```text
The structure of the config file is the same, but values differ per host, group, or environment.
```

Vault solves this problem:

```text
The playbook needs a secret, but the secret must not be stored as readable text.
```
...
Never print secrets with `debug`. Add `no_log: true` to tasks that handle passwords, private keys, or tokens.
...
# Chapter 14 — Reviewed Edition Closing Checkpoint
