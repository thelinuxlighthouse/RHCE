...
## Added Review: Choose the Right File Module

Use the smallest safe tool for the job:

| Need | Best module |
|---|---|
| Create directory or set permissions | `ansible.builtin.file` |
| Copy static file | `ansible.builtin.copy` |
| Generate config from variables | `ansible.builtin.template` |
| Change one known line | `ansible.builtin.lineinfile` |
| Manage a marked multi-line block | `ansible.builtin.blockinfile` |
| Download from URL | `ansible.builtin.get_url` |
| Collect file from managed node | `ansible.builtin.fetch` |
| Validate generated config before replacing | `copy` or `template` with `validate:` |

The safe pattern for service configuration is:

```text
install package -> deploy validated config -> notify handler -> start/enable service -> verify service and port
```

Do not restart services unconditionally after every run. Use handlers so services restart only when configuration changes.
...
6. **Use `become: true` for file deployment**: Most file deployments require root privileges.
...
# Chapter 07 — Reviewed Edition Closing Checkpoint
