...
## Added Review: SSH Key Authentication Without Mystery

SSH key authentication means: the control node keeps the private key, and the managed node stores the matching public key in `authorized_keys`.

The private key proves identity. The public key allows the managed node to recognize that identity. Never copy the private key to managed nodes.
...
ssh ansible@servera.lab.example.com 'whoami && hostname'
ansible servera.lab.example.com -m ansible.builtin.ping
```

When SSH fails, troubleshoot in this order:

1. hostname or IP is wrong;
2. SSH service is down;
3. firewall blocks TCP/22;
4. wrong remote user;
5. wrong key path;
6. permissions on `~/.ssh` or `authorized_keys` are unsafe;
7. sudo/become is failing, not SSH.
...
ansible servera -m systemd -a "name=sshd state=started enabled=true"
```
...
# Chapter 05 — Reviewed Edition Closing Checkpoint
