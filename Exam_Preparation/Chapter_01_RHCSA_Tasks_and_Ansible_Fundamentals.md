...
Most RHCSA tasks require root. Use `become: yes` in plays or individual tasks to escalate privileges.
...
This playbook demonstrates the `dnf` module, which replaces the manual `dnf install` and `dnf remove` commands. The `state: present` parameter ensures packages exist without upgrading unnecessarily. The `state: latest` parameter upgrades to the newest available version. The `dnf_module` module handles DNF module streams, which is essential for installing specific versions of software like PostgreSQL 16. The `update_cache: yes` parameter forces a metadata refresh, equivalent to running `dnf makecache`.
...
5. **Privilege escalation**: Most tasks require root. Always include `become: yes` at the play level unless a task explicitly should not run as root.
...
### Mistake 1: Forgetting `become: yes`
...
- `become: yes` is required for most system-level tasks
...
4. What is the purpose of the `become: yes` directive in an Ansible play?
...
4. `become: yes` enables privilege escalation, typically to root. Most system administration tasks require root privileges. Without this directive, tasks that modify system configuration, install packages, or manage services will fail with permission errors.
