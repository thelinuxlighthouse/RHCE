...
## Added Review: Git for Exam and Daily Ansible Work

For EX294 scope, do not get lost in advanced Git. Master this core loop:

```bash
git clone <repo-url>
cd <repo>
git status
git add playbook.yml roles/ inventory ansible.cfg
git commit -m 'Add automation for web servers'
git pull --rebase
git push
```
...
```text
Small change -> test -> commit -> explain clearly in commit message.
```

A good Ansible commit message says what state the playbook now enforces, not just which file changed.

Good:

```text
Configure httpd package, service, and firewall automation
```
...
# Chapter 09 — Reviewed Edition Closing Checkpoint
